import {CraftItem, CraftMaterial} from "~/types/crafting";
import {EnginePart} from "~/crafting/vehicle/parts/EnginePart";
import {TransmissionPart} from "~/crafting/vehicle/parts/TransmissionPart";
import {VehicleBodyPart} from "~/crafting/vehicle/parts/VehicleBodyPart";
import {InteriorPart} from "~/crafting/vehicle/parts/InteriorPart";
import {VehicleWindow} from "~/crafting/vehicle/parts/VehicleWindow";
import {Chassis} from "~/crafting/vehicle/parts/Chassis";
import {Tyre} from "~/crafting/vehicle/parts/Tyre";

export const Chevrolet_Camaro_ZL1_SSX = new CraftItem(
    'Chevrolet_Camaro_ZL1_SSX',
    'Chevrolet Camaro ZL1 SSX',
    20,
    [
        new CraftMaterial(9, EnginePart),
        new CraftMaterial(6, TransmissionPart),
        new CraftMaterial(5, VehicleBodyPart),
        new CraftMaterial(6, InteriorPart),
        new CraftMaterial(4, VehicleWindow),
        new CraftMaterial(9, Chassis),
        new CraftMaterial(4, Tyre),
        new CraftMaterial(1, new CraftItem('BP_Chevrolet_Camaro_ZL1_SSX', 'Bauplan Chevrolet Camaro ZL1 SSX', 20, []))
    ]
);